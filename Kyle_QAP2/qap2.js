 ...
 *
 *      Name: Kyle Hollett
 *      Date: June 10, 2024 Date finished: 2024-06-07
 */
...
function createVideo(src, width, controls) {
  ...
  if (width !== undefined && (Number.isInteger(width) || (typeof width === 'string' && !isNaN(parseInt(width))))) {
...
function parseDateString(value) {
  ...
  if (!match) {
    throw new Error('format should be YYYY-MM-DD');
  }
  ...
  if (isNaN(year) || isNaN(month) || isNaN(day)) {
    throw new Error('year month day must be numbers');
  }
  const date = new Date();
...
try {
  console.log(parseDateString('not-a-date'));
} catch (e) {
  console.error(e.message);
}
try {
  console.log(parseDateString('2024-10-10'));
} catch (e) {
  console.error(e.message);
}
/*******************************************************************************
...
try { 
  const coord1 = normalizeCoord('42.9755,-77.4369');
console.log(coord1);

const coord2 = normalizeCoord('[-77.4369, 42.9755]');
console.log(coord2);
} catch (e) {
  console.error(e.message);
}
...
function generateLicenseLink(licenseCode, targetBlank) {
  ...
  if (licenseCode in licenseMap) {
    description = licenseMap[licenseCode];
...
console.log(any('n', 'no', 'NON'));
console.log(none('n', 'no', 'NON'));
...
console.log(buildUrl('Horse-chestnut', 'descending', 10, 'cc-by-nc-sa')); 

try {
  console.log(buildUrl('Horse-chestnut', 'random', 10, 'cc-by-nc-sa'));
} catch (error) {
  console.error(error.message);
}

try {
  console.log(buildUrl('Horse-chestnut', 'ascending', 51, 'cc-by-nc-sa'));
} catch (error) {
  console.error(error.message);
}

try {
  console.log(buildUrl('Horse-chestnut', 'ascending', 10, 'invalid-license'));
} catch (error) {
  console.error(error.message);
}
